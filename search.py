...
class BST():
    def __init__(self,l=[]):
        ...
        for i in l:
            self.l_ord.append(self._ord_val(i))
            print(self.l_ord)
            self.add(i)
    ...
    def add(self,item,p=None):
        ...
        if c in self.l_ord:
            ...
            if self.root._item==None:
                self.addroot(item)
            elif c < self.l_ord[self.l.index(self.item(p))]:      #item=self.left(p)
                                                                    # ind in l: self.l.index(item)
                return self.addleft(item,p)                       # val in ord: self._ord_val[index]
            else:
    ...
    def search(self,item,p=0):
        ...
        if item==self.item(p):
            print("Search",item)
        elif self.l_ord[self.l.index(item)] < self.l_ord[self.l.index(self.item(p))]:
