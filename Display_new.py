...
class Display() :
    ...
    def __init__(self,root) :
        ...
        with open("Stock.csv", "r") as f :
            ...
            for i in reader :
                ...
                if i == [] :
                    ...
                    print(i[1].split('-'))
                    self.stock.enqueue_rear(Vegetable(i[0],int(i[1].split('-')[0]),i[1].split('-')[1],float(i[2].split()[0]),i[2],i[3]))
        ...
        self.Products_frame.create_text(
            ...
            293.0,
            anchor="nw",
            text=f"Qty: {self.stock.getfront().getitem().getdisqty()}                     Price : {self.stock.getfront().getitem().getdisprice()}",
            fill="#000000",
            font=("Inter ", 21 * -1)
        )
    ...
    def _order_num(self):
        ...
        address = "Your order will be delievered soon"
        Label(self.root,
                text=address,
                font=("Segoe UI Semibold",12),
                bg='white'
        ...
        Label(self.root,
                text="Your order number is "f'{num}',
                font=("Segoe UI Semibold",12),
                bg='white'
                ).pack(pady=10)
    def _final(self) :
    ...
    def _pincode_check(self):
        print(">>>>",self.pincode.get()[:3])
        if str(self.pincode.get()[:3]) == '600':
            self._final_pay()
        else:
            messagebox.showinfo("PINCODE","Enter a valid pincode")

    def _pincode(self):
        #self.canvas.destroy()
        self.pay_screen=LabelFrame(table_root,text="PAYMENT OPTIONS",fg="black",font=("Segoe UI Semibold",10),bg='white')
        self.pay_screen.place(x=300,y=100,width=400,height=420)
        ...
        self.submit_btn = Button(
            ...
            relief=SOLID,
            cursor='hand2',
            command= lambda: self._pincode_check()
        )
        address.grid(row=2, column=1, pady=10, padx=20)
        self.pincode.grid(row=4, column=1, pady=10, padx=20)
        
        self.submit_btn.grid(row=6, column=1, pady=10, padx=20)
    ...
    def _pay(self):
        ...
        self.screen.place(x=200,y=350,height=100,width=555)
    ...
    def display_next(self,item=None) :
        ...
        self.Products_frame.create_text(
            ...
            293.0,
            anchor="nw",
            text=f"Qty: {self.display_item.getitem().getdisqty()}                      Price : {self.display_item.getitem().getdisprice()} ",
            fill="#000000",
            font=("Inter ", 21 * -1)
        )
    ...
    def _display_all(self):
        ...
        for veg in self.stock :
            ...
            self.qty_veg = Label(self.Item_frame,
                text=f"Qty: {veg.getdisqty()}           Price : Rs. {veg.getdisprice()} ",
                bd=1,
                font=('Segoe UI Semibold',20),
                justify="left",
                bg = 'white'
        ...
        self.show_one = PhotoImage(
            file=self.relative_to_assets("show_btn.png"))
        self._show_one = Button(self.canvas,
            image=self.show_one,
            borderwidth=0,
            highlightthickness=0,
            bg = 'white',
            command=lambda: self.show_one_(),
            relief="flat"
        )
        self._show_one.place(
            x=775.0,
            y=258.0,
            width=114.0,
            height=41.94915771484375
